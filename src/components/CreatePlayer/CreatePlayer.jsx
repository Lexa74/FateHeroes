...
export const CreatePlayer = () => {
    ...
    const [charac, setCharac] = useState('Никаких свойств');
    ...
    return  (
        <div className="create-player">
            ...
            <select className="create-player__form-element" onChange={(e) => setCharac(e.target.value)}>
                ...
                {factions.map((faction) => <option value={faction.character} key={faction.id}>{faction.name}</option>)}
            </select>
            <div><strong>{charac}</strong></div>
        </div>
    )
  }
